...
#define Neutral_Steer_angle  450
...
******************************/
#define MAX_R_ANGLE 370
#define MAX_L_ANGLE 530
...
{
	ros::init(argc, argv, "m_car_serial_control_km_node");
	ros::NodeHandle n;
	...
	ros::Subscriber sub3_wp_angle = n.subscribe(cmd_car_angle, 20, car_steer_callback);

	uart_fd = init_serial_port(com_port); 
	ros::Rate loop_rate(LOOP_RATE); //10.0HZ

	m_car_speed_float.data = 0.0;
