...
int main(int argc, char** argv)
{
	/////////////////// 카메라 image는 640x360으로 고정(jetson nano) ////////////////////////
			...
			*/	
			...
			else
			{
				...
				*/
				...
				int width  = mat_image_roi_crop.size().width;
				int height = mat_image_roi_crop.size().height;
				...
				{
					if(euclidean_distance_matrix[i] < 50) 
					{
				...
				printf("left  line_c line_d :%5.1lf  %5.1lf  \n", left_line_c,left_line_d);
				printf("right line_c line_d :%5.1lf  %5.1lf  \n\n",right_line_c,right_line_d);
				...
				if(left_line_cnt !=0)
				{
					...
					y3 = (double)ROI_WIDTH; 
					x3 = left_line_c * y3 + left_line_d;
					printf("left line : x3, y3 | %5.1lf  %5.1lf %5.1lf  %5.1lf  \n", x3,y3,left_line_c,left_line_d);
					circle(mat_image_roi_crop, Point(x3 , ROI_WIDTH), 20, Scalar(0, 255, 255), 1, 8, 0);
					line_left_x = x3;
				...
				if(right_line_cnt !=0)
				{
					...
					y3 = (double)ROI_WIDTH; 
					x3 = right_line_c * y3 + right_line_d;
					printf("right line : x3, y3 | %5.1lf  %5.1lf %5.1lf  %5.1lf  \n", x3,y3,right_line_c,right_line_d);
					line_right_x = x3;
					
					line(mat_image_roi_crop, Point(x1, y1), Point(x2, y2), Scalar(0, 255, 255), 2, LINE_AA);
					circle(mat_image_roi_crop, Point(x3 , ROI_WIDTH), 20, Scalar(255, 0, 0), 1, 8, 0);
				...
				{
					printf("Two Line detected!");
					if(line_left_x >= line_right_x)
					{
						double temp    = line_left_x;
						line_left_x    = line_right_x;
						line_right_x   = temp;
						int temp2      = left_line_cnt;
						right_line_cnt = left_line_cnt;
						left_line_cnt  = temp2;
				...
				if( right_line_cnt == 0 ) 
				{
					printf( "line_left_x :%lf %lf \n",line_left_x , line_right_x);
					
					if( line_left_x < (img_width/2) )
					{
						line_center = (line_left_x + line_left_x + lane_width)/2;
						printf("left line : %lf %lf\n", line_left_x , line_left_x + lane_width/2);
					}
					else
					{
						line_center = (line_left_x + line_left_x - lane_width)/2;
				...
				if( left_line_cnt == 0 ) 
				{
					...
					{
						line_center = (line_right_x + line_right_x-lane_width)/2;
						printf("right line : %lf %lf\n", line_right_x , line_right_x - lane_width/2);
					}
					printf("Lane Center : %4.1lf  %4.1lf  %4.1lf \n", line_left_x, line_right_x, line_center);
					line(mat_image_roi_crop, Point(line_center,0), Point(line_center, ROI_WIDTH*2),  Scalar( 147, 20,255), 1, LINE_AA);
				}
